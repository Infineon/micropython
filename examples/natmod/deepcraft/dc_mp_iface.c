...
static inline unsigned int _f2u(float f) {
    union { float f;
            unsigned int u;
    } v;
    v.f = f;
...
static inline float _u2f(unsigned int u) {
    union { float f;
            unsigned int u;
    } v;
    v.u = u;
...
// --- log2f ---
float log2f(float x) {
    return logf(x) * 1.4426950409f;
}

// --- log10f ---
float log10f(float x) {
    return logf(x) * 0.4342944819f;
}
...
float fabsf(float x) {
    return _u2f(_f2u(x) & 0x7FFFFFFFu);
}
...
float powf(float a, float b) {
    ...
    if (a < 0.0f) {
        int ib = (int)b;
        if ((float)ib != b) {
            return 0.0f;                 // non-integer exponent of negative base
        }
...
float sinf(float x) {
    ...
    switch (n & 3) {
        case 0:
            return _dc_sinf_kernel(r);
        case 1:
            return _dc_cosf_kernel(r);
        case 2:
            return -_dc_sinf_kernel(r);
        default:
            return -_dc_cosf_kernel(r);
...
float atan2f(float y, float x) {
    ...
    if (x < 0.0f && y >= 0.0f) {
        return atanf(y / x) + 3.14159265359f;
    }
    if (x < 0.0f && y < 0.0f) {
        return atanf(y / x) - 3.14159265359f;
    }
